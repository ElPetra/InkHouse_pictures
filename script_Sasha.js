...
function setLS(event) {
  let text = event.target
    .closest(".picture__info")
    .querySelector(".picture__name").innerText;
...
function delEl(event) {
  let text = event.target
    ...
    .querySelector(".picture__name").innerText;
  arr.splice(
    arr.findIndex((el) => el.title == text),
    1
  );
  LS.setItem("artBasket", JSON.stringify(arr));
...
function createCard(obj) {
  ...
  card_text.innerHTML = `
    ...
    <button class="card__button ${obj.isAtBasket ? "addBasket" : ""}">${
    obj.isAtBasket ? "Добавлено" : "В корзину"
  }</button>`;
